...
def check_boycott():
    ...
    if company_name in boycott_brands:
        result_label.config(text=f"{company_name} is on the boycott list.")
    else:
        result_label.config(text=f"{company_name} is not on the boycott list.")
    entry1.delete(0, END)
...
def add_company():
    ...
    if company_name in boycott_brands:
        result_label.config(text=f"{company_name} is already on the boycott list.")
    else:
        with open(r'C:\Users\malak\Desktop\Boycott Brands.txt', 'a') as file:
            file.write(company_name + '\n')
        boycott_brands.append(company_name)  # Update the in-memory list
        result_label.config(text=f"{company_name} has been added to the boycott list.")
    entry2.delete(0, END)
...
label1 = Label(window, text="Company name:", font=('sansserif', 20), bg='light blue', fg='navy blue')  
...
label2 = Label(window, text="Company to add:", font=('sansserif', 20), bg='light blue', fg='navy blue')  
...
# Create and place the result label for output
result_label = Label(window, text="Result", font=('sansserif', 20), bg='light blue', fg='navy blue')
